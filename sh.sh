...
# Fix Devis - Formulaire complet avec produits multi-lignes
# Usage: ./fix-devis-form.sh

echo "Fix Devis formulaire..."

DEVIS_HTML="src/app/components/devis/devis.component.html"

# Backup
cp "$DEVIS_HTML" "${DEVIS_HTML}.backup.formfix"

# HTML complet avec formulaire multi-produits
cat > "$DEVIS_HTML" << 'EOFHTML'
<div class="container mx-auto px-4 py-8">
  <h1 class="text-3xl font-bold text-gray-800 mb-6">Devis</h1>
  ...
  <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
    <input type="text" [ngModel]="searchTerm" (ngModelChange)="onSearchChange($event)"
           placeholder="🔍 Rechercher par numéro..." class="w-full md:w-1/3 px-4 py-2 border rounded-lg" />
  ...
  <div class="bg-white shadow-md rounded-lg p-4 mb-6 no-print">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div>
        ...
        <select [ngModel]="statusFilter" (ngModelChange)="onStatusFilterChange($event)" class="w-full px-3 py-2 border rounded-lg">
    ...
    <form [formGroup]="devisForm" (ngSubmit)="onSubmit()" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <ng-select formControlName="customer" [items]="(customers$ | async) ?? []" bindLabel="name" bindValue="name" 
                     placeholder="-- Sélectionnez --" [searchable]="true" [clearable]="true" class="w-full"></ng-select>
        </div>
        <div>
          <label class="block text-gray-700 font-medium mb-2">Valide jusqu'au</label>
          <input formControlName="validUntil" type="date" class="w-full px-4 py-2 border rounded-lg" />
        </div>
      </div>
      ...
      <div class="border-t pt-4">
        ...
        <div class="mt-4 bg-violet-50 p-4 rounded">
          <div class="flex justify-between text-xl font-bold">
            <span>Total :</span>
            <span class="text-violet-600">{{ calculateTotal() | number:'1.2-2' }} DT</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        <button type="submit" [disabled]="isLoading || devisForm.invalid"
                class="bg-violet-600 hover:bg-violet-700 text-white font-bold py-2 px-6 rounded-lg disabled:bg-gray-400">
  ...
  <div *ngIf="!isLoading" class="space-y-4">
    <div *ngFor="let devis of filteredDevis$ | async" class="bg-white shadow rounded-lg overflow-hidden voucher-card" [attr.data-id]="devis.id">
      <div class="px-6 py-4 flex justify-between items-center cursor-pointer hover:bg-gray-50"
        ...
        <div class="flex-1 grid grid-cols-5 gap-4">
          ...
          <div><span [ngClass]="getStatusClass(devis.status || 'draft')" class="px-2 py-1 rounded text-xs">{{ getStatusLabel(devis.status || 'draft') }}</span></div>
          <div><strong>Total:</strong> {{ devis.totalAmount | number:'1.2-2' }} DT</div>
        </div>
        <button class="text-violet-600 no-print">{{ expandedDevisId === devis.id ? '▼' : '►' }}</button>
      </div>
      ...
      <div *ngIf="expandedDevisId === devis.id" class="px-6 py-4 bg-gray-50 border-t">
        <table class="min-w-full">
          ...
          <tbody>
            ...
            <tr class="bg-violet-50 font-bold">
              ...
              <td class="px-4 py-2 text-violet-600">{{ devis.totalAmount | number:'1.2-2' }} DT</td>
            </tr>
          </tbody>
        </table>
        <div class="mt-4 flex gap-2 no-print">
          ...
          <button (click)="printItem(devis)" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded">Imprimer Devis</button>
        </div>
      </div>
    </div>
...
echo "✅ Devis HTML régénéré avec formulaire complet"

echo ""
echo "=========================================="
echo "  ✅ Fix Devis Formulaire Terminé"
echo "=========================================="
echo "Formulaire Features :"
echo "  - Multi-produits (ajouter/supprimer lignes)"
echo "  - Sélection client (ng-select)"
echo "  - Date + Valide jusqu'au"
echo "  - Calcul total automatique"
echo "  - Statut (Brouillon/Envoyé/Accepté/Rejeté)"
echo ""
echo "Test :"
echo "  1. ng serve"
echo "  2. /devis → Click '+ Nouveau Devis'"
echo "  3. Formulaire s'affiche avec champs"
echo ""
