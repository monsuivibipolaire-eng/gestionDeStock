...
export class SuppliersComponent implements OnInit {
  ...
  private _selectedSupplierId: string | null = null;
  private selectedSupplier$ = new BehaviorSubject<string | null>(null);
  
  // Getter/Setter pour selectedSupplierId (auto-update BehaviorSubject)
  get selectedSupplierId(): string | null {
    return this._selectedSupplierId;
  }
  
  set selectedSupplierId(value: string | null) {
  ...
  ngOnInit(): void {
    ...
    this.filteredSuppliers$ = combineLatest([
      ...
      map(([suppliers, supplierId]) => {
        if (!supplierId) {
          return suppliers; // Si aucun sélectionné, montre tous
        }
        return suppliers.filter(s => s.id === supplierId); // Filtre exact par ID
      })
    );
  }
