...
export class EntryVoucherComponent implements OnInit {
  ...
  constructor(
    private vouchersService: EntryVouchersService,
  ...
  ngOnInit(): void {
    ...
    // Subscribe to products$ to populate productList for synchronous access
  ...
  // --- Data Loading ---
  loadVouchers(): void {
    this.vouchers$ = this.vouchersService.getEntryVouchers();
  }
  ...
  onSubmit(): void {
    ...
    const productsWithNames: ProductLine[] = formValue.products.map((p: any) => {
        // Recherche synchrone DANS le map
    ...
    if (this.isEditing && this.editingId) {
      this.vouchersService
        .updateEntryVoucher(this.editingId, voucherData)
        .then(() => {
          this.resetForm();
          this.loadVouchers();
        })
  ...
  generatePrintHTML(vouchers: EntryVoucher[]): void {
    ...
    const html = `
      <!DOCTYPE html><html><head>
        <meta charset="UTF-8"><title>Liste Bons d'Entrée - ${today}</title>
        <style> /* ... CSS Styles ... */
        ...
        <div class="header"><h1>📥 Bons d'Entrée - Liste</h1><p>Date d'impression : ${today}</p></div>
        <div class="filters"><h3>Filtres Appliqués</h3><ul>${filters.map((f) => `<li>• ${f}</li>`).join('')}</ul></div>
        <div class="stats">
        ...
        <table><thead><tr><th>#</th><th>N° Bon</th><th>Date</th><th>Fournisseur</th><th>Produits</th><th>Montant</th></tr></thead>
          <tbody>${rows}</tbody>
          <tfoot><tr><td colspan="4">TOTAL (${totalVouchers} bons)</td><td>${totalProducts}</td><td>${totalAmount.toFixed(2)} DT</td></tr></tfoot>
        </table>
      </body></html>
    `;
  ...
  printItem(item: any): void {
    this.filteredVouchers$.pipe(take(1)).subscribe((vouchers) => {
      ...
      const html = `
        <!DOCTYPE html><html><head>
          <meta charset="UTF-8"><title>Bon d'Entrée ${voucher.voucherNumber}</title>
          <style>
