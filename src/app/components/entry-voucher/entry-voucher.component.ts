...
export class EntryVoucherComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filteredVouchers$ = combineLatest([
      ...
      map(([vouchers, term, supplier, dateFrom, dateTo, minAmount, maxAmount, sortBy, sortOrder]) => {
        let filtered = vouchers;
        
        // Recherche numéro bon
        if (term) {
  ...
  onSubmit(): void {
    ...
    if (this.isEditing && this.editingId) {
      this.vouchersService.updateEntryVoucher(this.editingId, voucherData).then(() => {
        this.resetForm();
        this.loadVouchers();
      }).catch((err: any) => {
      ...
      this.vouchersService.addEntryVoucher(voucherData).then(() => {
        this.resetForm();
        this.loadVouchers();
      }).catch((err: any) => {
  ...
  deleteVoucher(id: string, voucherNumber: string): void {
    if (confirm(`Supprimer le bon "${voucherNumber}" ?`)) {
      this.isLoading = true;
      this.vouchersService.deleteEntryVoucher(id).then(() => {
        this.loadVouchers();
      }).catch((err: any) => {
