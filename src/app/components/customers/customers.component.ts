...
export class CustomersComponent implements OnInit {
  ...
  private selectedCustomer$ = new BehaviorSubject<string | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.filteredCustomers$ = combineLatest([
      ...
      map(([customers, customerId]) => {
        if (!customerId) {
          return customers; // Si aucun sélectionné, montre tous
        }
        return customers.filter(c => c.id === customerId); // Filtre exact par ID
      })
    );
  }
